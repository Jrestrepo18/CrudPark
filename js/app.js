    ...
    const initializeDashboard = async () => {
        ...
        const loadDashboardData = async () => {
            try {
                ...
                document.getElementById('daily-revenue').textContent = (data.ingresosTotales ?? 0).toLocaleString('es-CO', { style: 'currency', currency: 'COP', minimumFractionDigits: 0 });
                document.getElementById('active-memberships').textContent = data.mensualidadesActivas ?? '0';
                document.getElementById('expiring-memberships').textContent = data.proximasAVencer ?? '0';
                document.getElementById('expired-memberships').textContent = data.vencidas ?? '0';
    ...
    async function handleMembershipsView() {
        ...
        try {
            ...
            memberships.forEach(mem => {
                ...
                const hoy = new Date();
                const fechaFin = new Date(mem.fecha_fin);
                const diasRestantes = Math.ceil((fechaFin - hoy) / (1000 * 60 * 60 * 24));

                // Color de advertencia si está por vencer
                if (diasRestantes <= 5 && diasRestantes > 0) {
                    tr.className = 'border-b bg-yellow-100 hover:bg-yellow-200 transition';
                } else if (diasRestantes <= 0) {
                    tr.className = 'border-b bg-red-100 hover:bg-red-200 transition';
                } else {
                    tr.className = 'border-b hover:bg-gray-50';
                }
                ...
                tr.innerHTML = `
                    ...
                    <td class="p-4 text-right">
                        ...
                        <button class="send-reminder px-3 py-1 bg-indigo-600 text-white font-semibold rounded hover:bg-indigo-700 transition" data-id="${mem.id_mensualidad}">Enviar recordatorio</button>` : ''}
                    </td>
                `;
                tbody.appendChild(tr);
            });
            ...
            tbody.querySelectorAll('.send-reminder').forEach(btn => {
                btn.addEventListener('click', async () => {
                    const id = btn.dataset.id;
                    try {
                        await api.sendReminderEmail(id);
                        alert('Correo de recordatorio enviado correctamente.');
                        btn.disabled = true;
